...
% What is the most we can say about welfare since the there is apolicy focus?
% Better explain the insurance product (at least in the presentation)
% What are other ways subjects could have saved during the time of the study? (esp. mobile credit/savings)?
% The savings variable is a stock so say account balance.
% Explain why we use a lockbox account and explain how we might expect this to affect results/generalizability. Don't assume people are aware of this feature.
...
% The real story is that regret induces people to use the account at least once and that there is no effect on the intensive margin (there is)
