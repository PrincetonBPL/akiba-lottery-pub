...
\pagestyle{fancy}
\fancyhf{} \chead{\textsc{Using Lotteries to Encourage Saving}} \rhead{\thepage}
\linespread{1}
\urlstyle{tt}
...
\section{Experimental Design} \label{sec:design}
	...
	\subsection{Context and Sample Frame}
		...
		% last graf can be a footnote to how people gamble
	...
	\subsection{Data Collection}
		...
		% this might not be necessary, just talk about what measures we use from this
...
\section{Empirical Strategy} \label{sec:est}
	...
	\subsection{Minimum Detectable Effect Sizes}
		...
		% scrap this section; just talk about whether sizes we can detect will be econmically meaningful
	...
	\subsection{Time-Varying Treatment Effects}
		...
		%
		% \begin{equation} \begin{split}
		% Y_{i,t} = & \beta_{0} + \beta_{1}\text{Lottery}_{i} + \beta_{2}\text{Regret}_{i} + \lambda_{0}t \\
		% 			& + \lambda_{1}(\text{Lottery}_{i} \times t) + \lambda_{2}(\text{Regret}_{i} \times t) + \varepsilon_{i}
		...
		\begin{equation} \begin{split}
		Y_{i,t} = & \beta_{0} + \beta_{1}\text{Lottery}_{i} + \beta_{2}\text{Regret}_{i} \\
					& + \sum_{t=2}^{60} \bigg [ \zeta_{t} \tau_{t} + \eta_{t}(\text{Lottery}_{i} \times \tau_{t}) + \theta_{t}(\text{Regret}_{i} \times \tau_{t}) \bigg ] + u_{i, t}
		\end{split} \label{eq:timedummy} \end{equation}

		$Y_{i, t}$ is the outcome for individual $i$ at period $t$, Lottery$_i$ indicates assignment to the Lottery group, and Regret$_i$ indicates assignment to the Regret group. $\tau_t$ is an indicator taking the value 1 in period $t$. The omitted group is the control group at $t=1$. We can interpret the value of $\beta_1 + \eta_t$ and $\beta_2 + \theta_t$ as the effects of Lottery and Regret, respectively, at period $t$. We test the null hypothesis of no time-varying treatment effects with a joint tests of $\beta_1 + \eta_t = 0$ and $\beta_2 + \theta_t = 0$, respectively, for $t = 2, \ldots, 60$. Standard errors are clustered at the individual level.
