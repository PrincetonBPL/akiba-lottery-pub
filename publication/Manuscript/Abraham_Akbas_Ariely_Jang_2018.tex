...
\section{Results} \label{sec:results}
	...
	\subsection{PLS with Feedback Increases Deposit Frequency but Not Savings}
		...
		That potential savers respond to PLS by making more frequent deposits can be rationalized as the subdivision of lotteries to reduce risk \shortciteN{samuelson_risk_1963}. Under this hypothesis, risk averse individuals will subdivide bets over a greater number of gambles so that the risk of a low return is minimized. Since PLS offers strictly positive returns equivalent in expectation to the matching incentive, we would not expect any greater amount to be allocated to PLS than to the standard account even without liquidity constraints.\footnote{\shortciteN{samuelson_risk_1963} gives a proof that expected utilitu maximizers who would not take a single fair bet once would not take a sequence of such bets.} In this way, our results remain wholly consistent with expected utility maximization.
