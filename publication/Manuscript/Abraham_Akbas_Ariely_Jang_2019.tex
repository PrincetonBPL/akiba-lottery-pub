...
\section{Introduction}
	...
	% consider putting this discussion in the related literature section
...
\section{Experimental Design} \label{sec:design}
	...
	\subsection{Context and Sampling}
		...
		% we should show that savings can be profitable in this setting (positive NPV investments)
...
\section{Empirical Strategy} \label{sec:est}
	...
	\subsection{Testing for Regret Aversion}
		...
		We can leverage the panel structure of our data and the randomness of the lottery results to formulate a test of regret aversion. If participants are saving in response to experienced regret of foregoing the prize, we expect to see an effect of winning the lottery above and beyond the group effect. Let $Y_{i,t}$ denote having made a deposit or the amount deposited by participant $i$ in period $t$. $\text{Win}_{i,t}$ is an indicator for having a winning lottery ticket from the pervious day announced in period $t$. We estimate the following equation conditional on assignment to the Regret group and not having saved one period prior.

		\begin{equation} \begin{split}
		Y_{i,t} = & \pi \text{Win}_{i,t} + \omega_{t} + u_{i,t}
		\end{split} \label{eq:regret} \end{equation}

		$\pi$ is the additional effect from winning but not being able to claim the prize. $\omega_{t}$ is a period-specific fixed effect. We test the null hypothesis of no regret aversion, $\pi = 0$. Standard errors are clustered at the individual level.

		% do this overall only among the regret group and then plot the time trend of effects
		% the previous analysis in this section seems wrong
		% regret - lottery identifies the anticipated regret aversion but maybe we dont see effects due to power or time trend
		% motivates a look at the time trend of effects
