...
foreach yvar in $ymobile {
	...
	twoway (line C1 C5, color(gs0) lpattern(solid)) (line C3 C5, color(gs0) lpattern(dash)) (rcap ubl lbl C5, color(gs0)) (rcap ubr lbr C5, color(gs0)), ytitle("Effect point estimate") xtitle("CRRA parameter") legend(order(1 "No Feedback" 2 "PLS")) graphregion(color(white))
...
foreach yvar of varlist mobile_deposits mobile_depositamount {
	...
	gr tw (rarea ub1 lb1 period, color(gs12) sort) (line M1 period, color(gs0) lpattern(solid)), title("A: Effect of PLS without feedback over time", color(gs0) size(medsmall)) ytitle("Effect size") legend(off) graphregion(color(white)) ylabel(,glcolor(gs14)) saving("$fig_dir/line-timeeffectlottery", replace)

	gr tw (rarea ub2 lb2 period, color(gs12) sort) (line M3 period, color(gs0) lpattern(solid)), title("B: Effect of PLS over time", color(gs0) size(medsmall)) ytitle("Effect size") legend(off) graphregion(color(white)) ylabel(,glcolor(gs14)) saving("$fig_dir/line-timeeffectregret", replace)
