** Title: sum-eltreat.do
** Author: Justin Abraham
** Desc: Outputs baseline summary statistics conditional on treatment group for endline only
** Input: UMIP Master.dta
** Output: sum-eltreat.do
...
foreach yvar in $sumvars {
	...
	qui reg `yvar' lottery regret if endline == 1, vce(cl surveyid)
	est sto reg
	loc N = e(N)
	...
	qui su `yvar' if control == 1 & endline == 1
...
loc footnote "These results are restricted to the sample of participants who completed the endline survey. The first three columns report the difference of means across treatment groups with SEs in parentheses. Column 4 reports the mean of the control group with SD in parentheses. * denotes significance at 10 pct., ** at 5 pct., and *** at 1 pct. level."
...
file write tex _n "% File produced by sum-eltreat.do with `c(filename)' on `c(current_time)' `c(current_date)' by user `c(username)' on Stata `c(version)' with seed `c(seed)'"
file close tex
