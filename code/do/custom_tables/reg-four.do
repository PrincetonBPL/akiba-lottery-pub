...
loc countp = `count' + 2	// Cell second line
...
stepdown reg ($regvars) treated, iter($iterations) options(vce(cl surveyid))
mat F1 = r(p)

stepdown reg ($regvars) lottery if regret == 0, iter($iterations) options(vce(cl surveyid))
mat F2 = r(p)

stepdown reg ($regvars) regret if lottery == 0, iter($iterations) options(vce(cl surveyid))
mat F3 = r(p)

stepdown reg ($regvars) regret if control == 0, iter($iterations) options(vce(cl surveyid))
mat F4 = r(p)
...
foreach yvar in $regvars {
	...
	loc H1 = "([o_`yvar'_mean]regret + [o_`yvar'_mean]lottery) / 2"
	loc H2 = "[o_`yvar'_mean]lottery"
	loc H3 = "[o_`yvar'_mean]regret"
	loc H4 = "[o_`yvar'_mean]regret - [o_`yvar'_mean]lottery"
...
foreach yvar in $regvars {
	...
	forval i = 1/4 {
		...
		loc p_adj = F`i'[1, `varindex']
		sigstar, p(`p_adj') prec(2) pbracket pstar
	...
	estadd loc thisstat`count' = ``yvar'_N': col6
...
loc prehead_n "\begin{table}[ht]\centering \def\sym#1{\ifmmode^{#1}\else\(^{#1}\)\fi} \label{tab:$regpath} \maxsizebox*{\textwidth}{\textheight}{ \begin{threeparttable} \begin{tabular}{l*{`columns'}{c}} \toprule"
...
esttab col* using "$tab_dir/$regpath.tex", booktabs cells(none) nogap mgroups("Effect estimates" "Sample", pattern(1 0 0 0 1 0) prefix(\multicolumn{@span}{c}{) suffix(}) span erepeat(\cmidrule(lr){@span})) mtitle("Any lottery" "No feedback" "Feedback" "\specialcell{Difference}" "\specialcell{Control Mean\\(SD)}" "Obs.") stats(`statnames', labels(`varlabels')) note("`footnote'") prehead("`prehead'") postfoot("`postfoot'") compress replace
esttab col* using "$tab_dir/$regpath-n.tex", booktabs cells(none) nogap mgroups("Effect estimates" "Sample", pattern(1 0 0 0 1 0) prefix(\multicolumn{@span}{c}{) suffix(}) span erepeat(\cmidrule(lr){@span})) mtitle("Any lottery" "No feedback" "Feedback" "\specialcell{Difference}" "\specialcell{Control Mean\\(SD)}" "Obs.") stats(`statnames', labels(`varlabels')) prehead("`prehead_n'") postfoot("`postfoot'") compress replace

eststo clear

file open tex using "$tab_dir/$regpath.tex", write append
file write tex _n "% File produced by reg-four.do with `c(filename)' on `c(current_time)' `c(current_date)' by user `c(username)' on Stata `c(version)' with seed `c(seed)'"
file close tex
