...
esttab col* using "$tab_dir/$regpath.tex", booktabs cells(none) nogap mgroups("Relative risk ratio" "Sample", pattern(1 0 0 0 1) prefix(\multicolumn{@span}{c}{) suffix(}) span erepeat(\cmidrule(lr){@span})) mtitle("Constant" "PLS-N" "PLS-F" "\specialcell{PLS-F $-$ \\PLS-N}" "Obs.") stats(`statnames', labels(`varlabels')) note("`footnote'") prehead("`prehead'") postfoot("`postfoot'") compress replace
esttab col* using "$tab_dir/$regpath-n.tex", booktabs cells(none) nogap mgroups("Relative risk ratio" "Sample", pattern(1 0 0 1) prefix(\multicolumn{@span}{c}{) suffix(}) span erepeat(\cmidrule(lr){@span})) mtitle("Constant" "PLS-N" "PLS-F" "\specialcell{PLS-F $-$ \\PLS-N}" "Obs.") stats(`statnames', labels(`varlabels')) prehead("`prehead_n'") postfoot("`postfoot'") compress replace
