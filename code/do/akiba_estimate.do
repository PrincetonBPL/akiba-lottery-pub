...
if $heteffectsflag {
	...
	foreach xvar in $xhet {
		...
		la var LX`xvar' "PLS-N $\times$ \\ `xvarlab'"

		gen RX`xvar' = regret * `xvar'
		la var RX`xvar' "PLS-F $\times$ \\ `xvarlab'"
...
esttab using "$tab_dir/reg-timedummy", alignment(c) ar2 nobaselevels nogap label obslast nonum b(%9.2f) se(%9.2f) sfmt(%9.2f) drop(*.period) scalars("jointp2 PLS-N joint \(p\)-value" "jointp3 PLS-F joint \(p\)-value") star(* 0.10 ** 0.05 *** 0.01) note("`footnote'") prehead("`prehead'") postfoot("`postfoot'") se compress booktabs replace
eststo clear
